...
//        pp.testiTest(txt2);
    ...
    public static void testSimpleFeatureGen() {
        ...
        CoreDocument document = pipeline.processToCoreDocument(txt2);

        GeneratorFactory factory = new GeneratorFactory();

//        var generator = factory.getGenerator(Feature.FIRST_PERSON_PRONOUNS);
        var generator = factory.getGenerator(Feature.SECOND_PERSON_PRONOUNS);
        FeatureResult result = generator.getFeatureValue(document);

        util.showTextWithAnno(document.tokens().stream().map(CoreLabel::word).toList(),
                              result.annotation);
